...
void main() {
  ...
  const assetPath = 'assets/content/e3rab_general_majrourat_batch1_v1.json';
  const coveragePath = 'assets/content/e3rab_grammar_coverage_v2.json';
  const topics = [
    'majrourat.preposition',
    'majrourat.genitive-construction',
    'majrourat.genitive-follower',
    'majrourat.preposition-meanings',
  ...
  test('majrourat pack is valid and follows prerequisite order', () async {
    ...
    expect(
      ...
      orderedEquals([700, 710, 720, 730, 740]),
    );
    ...
    expect(lessons.last['id'], 'majrourat-genitive-types');
  });

  test('majrourat topics match comprehensive coverage', () async {
    final pack = await load(assetPath);
    final coverage = await load(coveragePath);
    final track = (coverage['tracks'] as List)
        .map((item) => Map<String, dynamic>.from(item as Map))
        .singleWhere((item) => item['id'] == 'majrourat');
    final lessonTopics = (pack['lessons'] as List)
        .map((lesson) => (lesson as Map)['topicId'])
        .toList(growable: false);

    expect(track['order'], 8);
    expect(track['topicIds'], topics);
    expect(lessonTopics, topics);
  });

  test('majrourat examples and entities use unique IDs', () async {
    final pack = await load(assetPath);
    final manifest = Map<String, dynamic>.from(pack['manifest'] as Map);
